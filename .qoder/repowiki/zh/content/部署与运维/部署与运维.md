...
### 健康检查与就绪检查
- 健康端点：/health返回200 OK，用于存活探针。
- 就绪检查：/readyz端点执行轻量级GCS连接性检查，确保代理已准备好接收流量。
- 优雅停机：监听SIGTERM/SIGINT，最多等待10秒优雅关闭。
...
## 结论
S3Proxy4GCS通过"控制面翻译+数据面直通"的混合架构，在保持S3语义一致性的同时，高效对接GCS能力。生产部署建议结合Cloud Run/GKE进行弹性伸缩，并通过结构化日志与健康检查实现可观测性；配合Prometheus监控指标、连接池与超时参数、客户端侧优化与版本互操作策略，可获得稳定且高性能的运行效果。
