# 技术架构

<cite>
**本文引用的文件**
- [main.go](file://main.go)
- [config/settings.go](file://config/settings.go)
- [pkg/translate/s3_cors.go](file://pkg/translate/s3_cors.go)
- [pkg/translate/gcs_cors.go](file://pkg/translate/gcs_cors.go)
- [pkg/translate/s3_lifecycle.go](file://pkg/translate/s3_lifecycle.go)
- [pkg/translate/gcs_lifecycle.go](file://pkg/translate/gcs_lifecycle.go)
- [pkg/translate/s3_tagging.go](file://pkg/translate/s3_tagging.go)
- [pkg/translate/gcs_tagging.go](file://pkg/translate/gcs_tagging.go)
- [pkg/translate/gcs_website.go](file://pkg/translate/gcs_website.go)
- [go.mod](file://go.mod)
- [README.md](file://README.md)
- [integration_tests/test_utils.go](file://integration_tests/test_utils.go)
</cite>

## 更新摘要
**变更内容**
- 新增可观测性中间件，替换默认日志记录器为结构化JSON日志
- 集成Prometheus指标收集，包括HTTP请求总数、请求持续时间、GCS API调用时长
- 新增/metrics端点用于暴露监控指标
- 增强GCS API调用的计时和指标记录
- 更新架构图以反映新的可观测性组件

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [组件详解](#组件详解)
6. [依赖关系分析](#依赖关系分析)
7. [性能与可靠性特性](#性能与可靠性特性)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本项目是一个面向 GCS 的 S3 兼容代理，通过反向代理与翻译器模块，将 S3 客户端的请求透明地转换为 GCS 可接受的格式，并在必要时进行签名重签与版本互操作映射。系统支持生命周期、CORS、日志、网站托管与对象标签等扩展能力的双向翻译，并提供连接池优化、优雅关闭、超时控制等工程化特性，便于在生产环境中稳定运行。

**更新** 新增了完整的可观测性体系，包括结构化日志、Prometheus指标收集和监控端点，为生产环境提供了强大的监控和诊断能力。

## 项目结构
- 根目录入口：main.go 提供 HTTP 路由、反向代理、请求拦截与处理器注册，以及可观测性中间件。
- 配置模块：config/settings.go 提供集中式环境变量加载与默认值设置。
- 翻译器模块：pkg/translate 下按功能划分的 S3/GCS 数据模型与双向翻译逻辑。
- 依赖声明：go.mod 明确第三方库与版本约束，包括 Prometheus 客户端库。
- 文档与测试：README.md 提供使用说明；integration_tests 子模块用于集成测试。

```mermaid
graph TB
subgraph "应用层"
R["HTTP 路由<br/>main.go"]
H["请求处理器<br/>handleS3Request 等"]
MW["可观测性中间件<br/>observabilityMiddleware"]
END["操作端点<br/>/health /readyz /metrics"]
end
subgraph "中间件层"
RP["反向代理<br/>httputil.ReverseProxy"]
DIR["Director/ModifyResponse<br/>重定向与响应头映射"]
ENDM["基础中间件<br/>RequestID/Recoverer"]
end
subgraph "配置层"
CFG["配置加载<br/>config/settings.go"]
end
subgraph "翻译层"
T1["生命周期翻译<br/>pkg/translate/gcs_lifecycle.go"]
T2["CORS 翻译<br/>pkg/translate/gcs_cors.go"]
T3["网站托管翻译<br/>pkg/translate/gcs_website.go"]
T4["标签翻译<br/>pkg/translate/gcs_tagging.go"]
end
subgraph "外部服务"
GCS["GCS API/SDK"]
PROM["Prometheus Server"]
end
R --> ENDM
ENDM --> MW
MW --> H
H --> RP
RP --> DIR
H --> T1
H --> T2
H --> T3
H --> T4
RP --> GCS
T1 --> GCS
T2 --> GCS
T3 --> GCS
T4 --> GCS
END --> PROM
CFG --> R
```

**图示来源**
- [main.go:325-362](file://main.go#L325-L362)
- [main.go:231-237](file://main.go#L231-L237)
- [main.go:268](file://main.go#L268)
- [main.go:39-64](file://main.go#L39-L64)

**章节来源**
- [main.go:1-1023](file://main.go#L1-L1023)
- [config/settings.go:1-65](file://config/settings.go#L1-L65)
- [go.mod:1-68](file://go.mod#L1-L68)
- [README.md:89-175](file://README.md#L89-L175)

## 核心组件
- 配置中心：集中加载环境变量与默认值，支持端口、目标桶、DryRun、连接池参数、代理凭据与 JSON 密钥路径等。
- 反向代理：基于标准库 httputil.NewSingleHostReverseProxy，自定义 Director 与 ModifyResponse，完成 S3 头部重写、签名重签、版本互操作映射与响应头转换。
- 请求拦截器：根据查询参数识别生命周期、CORS、日志、网站、标签等扩展请求，交由对应处理器处理；其余请求透传至 GCS。
- 翻译器：提供 S3 XML 与 GCS JSON/SDK 类型之间的双向转换，覆盖生命周期、CORS、网站托管与对象标签。
- **可观测性中间件**：结构化JSON日志记录器，包含请求ID、方法、路径、状态、持续时间、内容长度等关键信息，并自动记录Prometheus指标。
- **Prometheus指标系统**：包括HTTP请求总数、请求持续时间直方图、GCS API调用时长直方图等监控指标。
- **监控端点**：/metrics 端点暴露所有注册的Prometheus指标，支持外部监控系统抓取。
- **结构化日志系统**：使用 log/slog 输出结构化JSON日志，支持Info和Debug级别，便于云平台日志聚合。
- 运行时控制：优雅关闭、信号处理、超时控制与连接池参数调优。

**更新** 新增了完整的可观测性体系，包括结构化日志、Prometheus指标收集和监控端点，为生产环境提供了强大的监控和诊断能力。

**章节来源**
- [config/settings.go:11-25](file://config/settings.go#L11-L25)
- [main.go:325-362](file://main.go#L325-L362)
- [main.go:39-64](file://main.go#L39-L64)
- [main.go:268](file://main.go#L268)
- [README.md:169-170](file://README.md#L169-L170)

## 架构总览
系统采用"路由分发 + 反向代理 + 翻译器 + 可观测性"的分层架构：
- 路由层：基于 chi 路由注册健康检查、就绪检查、监控端点与通配符处理器。
- 中间件层：基础中间件（RequestID、Recoverer）之上，新增可观测性中间件，负责结构化日志记录和指标收集。
- 中间层：反向代理负责将请求转发到 GCS，并在 Director 中执行头部重写、签名重签与版本互操作注入。
- 翻译层：针对特定扩展请求（如 lifecycle/cors/logging/website/tagging）解析 S3 XML，转换为 GCS JSON 或 SDK 类型，再通过官方 SDK 更新 GCS。
- 配置层：统一从环境变量加载配置，支持 DryRun 模式与连接池参数调优。
- 监控层：集成Prometheus指标收集，提供/metrics端点，支持外部监控系统。

```mermaid
sequenceDiagram
participant C as "S3 客户端"
participant P as "代理服务器(main.go)"
participant OM as "可观测性中间件"
participant RP as "反向代理(httputil)"
participant TR as "翻译器(pkg/translate)"
participant G as "GCS"
participant M as "Prometheus"
C->>P : 发送 S3 请求
P->>OM : 进入可观测性中间件
OM->>OM : 记录请求开始时间
OM->>P : 调用下一个处理器
P->>P : 解析查询参数/路由匹配
alt 扩展请求(lifecycle/cors/logging/website/tagging)
P->>TR : 解析S3 XML并翻译为GCS JSON/SDK类型
TR->>G : 通过GCS SDK更新桶属性或对象元数据
G-->>TR : 返回结果
TR-->>P : 返回翻译后的响应
P-->>OM : 返回响应给可观测性中间件
OM->>M : 增加HTTP请求计数器
OM->>M : 记录请求持续时间
OM->>OM : 记录结构化日志
OM-->>C : 返回S3兼容响应
else 标准对象请求
P->>RP : 交由反向代理转发
RP->>G : 转发到GCS(可能重签/头重写/版本互操作)
G-->>RP : 返回GCS响应
RP-->>P : 修改响应头(版本互操作映射)
P-->>OM : 返回响应给可观测性中间件
OM->>M : 增加HTTP请求计数器
OM->>M : 记录请求持续时间
OM-->>C : 返回S3兼容响应
end
```

**图示来源**
- [main.go:325-362](file://main.go#L325-L362)
- [main.go:373-386](file://main.go#L373-L386)
- [main.go:268](file://main.go#L268)

## 组件详解

### 反向代理与请求拦截
- 初始化：解析 STORAGE_BASE_URL 为 GCS 基础地址，创建单主机反向代理。
- Director：重写 Host/URL Scheme，处理 Content-Length，执行存储类映射、x-id 参数剥离、Accept-Encoding 规避、版本互操作注入，并在需要时进行 AWS4-HMAC 重签。
- ModifyResponse：在入站响应中将 x-goog-generation 映射为 x-amz-version-id，实现版本互操作。
- 传输层：启用 HTTP/2、禁用压缩以保留 S3 签名所需的 Accept-Encoding、设置空闲连接数与超时，提升吞吐与稳定性。
- 路由：注册 /health 健康检查、/readyz 就绪检查、/metrics 监控端点与通配符处理器，拦截 ?lifecycle/?cors/?logging/?website/?tagging 查询参数，其余请求透传。

```mermaid
flowchart TD
Start(["进入 Director"]) --> Host["重写 Host/URL.Scheme"]
Host --> CL["解析 Content-Length"]
CL --> SC{"检测存储类非 STANDARD?"}
SC --> |是| MapSC["映射为 GCS 存储类并标记重签"]
SC --> |否| Next1["继续"]
MapSC --> Next1
Next1 --> XID{"检测 x-id 查询参数?"}
XID --> |是| StripXID["删除 x-id 并标记重签"]
XID --> |否| Next2["继续"]
StripXID --> Next2
Next2 --> AE{"检测 Accept-Encoding=identity?"}
AE --> |是| StripAE["删除该头并标记重签"]
AE --> |否| Next3["继续"]
StripAE --> Next3
Next3 --> VERS{"检测 versions 查询?"}
VERS --> |是| InjectV["注入版本互操作头"]
VERS --> |否| Next4["继续"]
InjectV --> Next4
Next4 --> NeedResign{"是否需要重签?"}
NeedResign --> |是| Resign["使用代理凭据重签"]
NeedResign --> |否| EndDir["结束 Director"]
Resign --> EndDir
```

**图示来源**
- [main.go:126-216](file://main.go#L126-L216)

**章节来源**
- [main.go:101-125](file://main.go#L101-L125)
- [main.go:126-216](file://main.go#L126-L216)
- [main.go:218-229](file://main.go#L218-L229)
- [main.go:231-278](file://main.go#L231-L278)

### 可观测性中间件与结构化日志
- **结构化JSON日志**：使用 log/slog 输出结构化日志，包含 request_id、method、uri、status、duration_ms、content_length、handler 等关键字段，支持 Info 和 Debug 级别。
- **请求追踪**：利用 chi 的 RequestID 中间件获取唯一请求ID，确保跨组件的日志关联性。
- **指标收集**：自动记录 HTTP 请求总数、请求持续时间分布，按 method、handler、status 分类统计。
- **处理器标签**：自动识别请求处理器类型（proxy、lifecycle、cors、logging、website、tagging），便于指标分类分析。
- **日志级别控制**：通过 DEBUG_LOGGING 环境变量控制日志详细程度，生产环境建议使用默认Info级别。

```mermaid
flowchart TD
A["进入 observabilityMiddleware"] --> B["记录开始时间"]
B --> C["包装 ResponseWriter 捕获状态码"]
C --> D["调用下一个处理器"]
D --> E["计算请求持续时间"]
E --> F["提取请求ID"]
F --> G["识别处理器类型"]
G --> H["更新 HTTP 请求计数器"]
H --> I["记录请求持续时间直方图"]
I --> J["输出结构化JSON日志"]
J --> K["返回响应"]
```

**图示来源**
- [main.go:325-362](file://main.go#L325-L362)

**章节来源**
- [main.go:74-80](file://main.go#L74-L80)
- [main.go:325-362](file://main.go#L325-L362)
- [main.go:365-371](file://main.go#L365-L371)

### Prometheus指标系统
- **HTTP请求总数**：s3proxy_http_requests_total，标签包括 method、handler、status，用于统计请求量和错误率。
- **HTTP请求持续时间**：s3proxy_http_request_duration_seconds，直方图类型，按 method、handler 分类记录请求耗时分布。
- **GCS API调用时长**：s3proxy_gcs_api_duration_seconds，直方图类型，记录各类GCS操作的执行时间。
- **指标注册**：在 init 函数中注册所有指标，确保启动时可用。
- **监控端点**：/metrics 端点暴露所有注册指标，支持 Prometheus 抓取。

```mermaid
classDiagram
class PrometheusMetrics {
+httpRequestsTotal : CounterVec
+httpRequestDuration : HistogramVec
+gcsAPIDuration : HistogramVec
+registerMetrics()
}
class ObservabilityMiddleware {
+recordMetrics()
+logStructured()
}
class TimeGCSCall {
+observeDuration()
+logOperation()
}
PrometheusMetrics --> ObservabilityMiddleware : "提供指标实例"
ObservabilityMiddleware --> PrometheusMetrics : "更新HTTP指标"
TimeGCSCall --> PrometheusMetrics : "更新GCS指标"
```

**图示来源**
- [main.go:39-64](file://main.go#L39-L64)
- [main.go:66](file://main.go#L66)
- [main.go:268](file://main.go#L268)

**章节来源**
- [main.go:39-64](file://main.go#L39-L64)
- [main.go:66](file://main.go#L66)
- [main.go:268](file://main.go#L268)

### 配置管理
- 支持 .env 文件与环境变量混合加载，优先使用环境变量。
- 关键配置项：端口、GCP 项目、目标桶、GCS 基础 URL、前缀、DryRun、调试日志、连接池上限、代理访问密钥、代理密钥、JSON 密钥路径。
- 默认值：DRY_RUN 默认开启（安全），MAX_IDLE_CONNS/MAX_IDLE_CONNS_PER_HOST 默认较大以提升吞吐。

```mermaid
flowchart TD
A["启动 LoadConfig"] --> B{"是否存在 .env?"}
B --> |是| C["加载 .env"]
B --> |否| D["直接读取环境变量"]
C --> E["解析各配置项"]
D --> E
E --> F["设置默认值(如 DRY_RUN/CONNS)"]
F --> G["赋值全局 Config"]
```

**图示来源**
- [config/settings.go:29-57](file://config/settings.go#L29-L57)

**章节来源**
- [config/settings.go:11-25](file://config/settings.go#L11-L25)
- [config/settings.go:29-57](file://config/settings.go#L29-L57)
- [README.md:18-29](file://README.md#L18-L29)

### 生命周期翻译器
- 输入：S3 LifecycleConfiguration XML。
- 输出：GCS JSON 字节流，或反向转换为 S3 XML。
- 支持规则：过期（Delete）、过渡（SetStorageClass）、非当前版本过期（Delete + IsLive=false + NumNewerVersions）。
- 不支持过滤：ObjectSizeGreaterThan/LessThan、Tag 过滤在 S3 到 GCS 翻译中被拒绝。
- 存储类映射：STANDARD_IA/ONEZONE_IA→NEARLINE，INTELLIGENT_TIERING→STANDARD，GLACIER/GLACIER_IR→COLDLINE，DEEP_ARCHIVE→ARCHIVE。
- 日期格式：S3 日期字符串截断为 yyyy-mm-dd。

```mermaid
flowchart TD
S3["S3 Lifecycle XML"] --> Parse["解析为结构体"]
Parse --> Rules["遍历规则(Status=Enabled)"]
Rules --> Exp{"存在 Expiration?"}
Exp --> |是| AddDel["生成 Delete 动作规则"]
Exp --> |否| Trans["继续"]
AddDel --> Trans
Trans --> TransList["遍历 Transitions"]
TransList --> MapSC["映射存储类"]
MapSC --> AddTrans["生成 SetStorageClass 规则"]
AddTrans --> NVE{"存在 NoncurrentVersionExpirations?"}
NVE --> |是| AddNVE["生成 Delete 动作 + IsLive=false + NumNewerVersions"]
NVE --> |否| Done["完成"]
AddNVE --> Done
Done --> JSON["输出 GCS JSON"]
```

**图示来源**
- [pkg/translate/s3_lifecycle.go:7-78](file://pkg/translate/s3_lifecycle.go#L7-L78)
- [pkg/translate/gcs_lifecycle.go:38-105](file://pkg/translate/gcs_lifecycle.go#L38-L105)
- [pkg/translate/gcs_lifecycle.go:107-137](file://pkg/translate/gcs_lifecycle.go#L107-L137)
- [pkg/translate/gcs_lifecycle.go:139-154](file://pkg/translate/gcs_lifecycle.go#L139-L154)

**章节来源**
- [pkg/translate/s3_lifecycle.go:7-78](file://pkg/translate/s3_lifecycle.go#L7-L78)
- [pkg/translate/gcs_lifecycle.go:38-105](file://pkg/translate/gcs_lifecycle.go#L38-L105)
- [pkg/translate/gcs_lifecycle.go:107-137](file://pkg/translate/gcs_lifecycle.go#L107-L137)
- [pkg/translate/gcs_lifecycle.go:139-154](file://pkg/translate/gcs_lifecycle.go#L139-L154)

### CORS 翻译器
- S3 CORS XML → GCS storage.CORS 切片：忽略 S3 的 AllowedHeaders（GCS 不原生支持），保留 MaxAge、Methods、Origins、ExposeHeaders。
- GCS CORS → S3 CORS XML：反向映射，将 MaxAge 转换为秒。

```mermaid
classDiagram
class S3CORS {
+CORSRules[] : CORSRule
}
class CORSRule {
+ID
+AllowedHeaders[]
+AllowedMethods[]
+AllowedOrigins[]
+ExposeHeaders[]
+MaxAgeSeconds
}
class GCSCORS {
+MaxAge
+Methods[]
+Origins[]
+ResponseHeaders[]
}
S3CORS --> CORSRule
CORSRule --> GCSCORS : "映射"
```

**图示来源**
- [pkg/translate/s3_cors.go:5-19](file://pkg/translate/s3_cors.go#L5-L19)
- [pkg/translate/gcs_cors.go:10-35](file://pkg/translate/gcs_cors.go#L10-L35)

**章节来源**
- [pkg/translate/s3_cors.go:5-19](file://pkg/translate/s3_cors.go#L5-L19)
- [pkg/translate/gcs_cors.go:10-35](file://pkg/translate/gcs_cors.go#L10-L35)

### 网站托管翻译器
- S3 WebsiteConfiguration → GCS storage.BucketWebsite：映射 IndexDocument.Suffix 为主页后缀，ErrorDocument.Key 为 404 页面。
- GCS → S3：反向映射主页后缀与 404 页面。

```mermaid
classDiagram
class S3Website {
+IndexDocument
+ErrorDocument
}
class GCSWebsite {
+MainPageSuffix
+NotFoundPage
}
S3Website --> GCSWebsite : "映射"
```

**图示来源**
- [pkg/translate/gcs_website.go:9-26](file://pkg/translate/gcs_website.go#L9-L26)

**章节来源**
- [pkg/translate/gcs_website.go:9-26](file://pkg/translate/gcs_website.go#L9-L26)

### 对象标签翻译器
- S3 Tagging → GCS Metadata：以 s3tag- 为前缀写入键，先清理旧 s3tag- 键，再写入新键值，实现幂等更新。
- GCS Metadata → S3 Tagging：过滤 s3tag- 前缀键，还原为 S3 TagSet。
- 使用对象元数据更新接口，结合乐观并发控制（IfMetagenerationMatch）避免覆盖冲突。

```mermaid
flowchart TD
A["S3 Tagging XML"] --> B["解析 TagSet"]
B --> C["清理旧 s3tag- 键(置空)"]
C --> D["写入新 s3tag- 键值"]
D --> E["返回更新映射"]
E --> F["GCS 对象元数据更新(OCC)"]
```

**图示来源**
- [pkg/translate/gcs_tagging.go:10-35](file://pkg/translate/gcs_tagging.go#L10-L35)

**章节来源**
- [pkg/translate/s3_tagging.go:5-10](file://pkg/translate/s3_tagging.go#L5-L10)
- [pkg/translate/gcs_tagging.go:10-35](file://pkg/translate/gcs_tagging.go#L10-L35)
- [main.go:874-940](file://main.go#L874-L940)

### 扩展请求处理器
- 生命周期：PUT/GET/DELETE ?lifecycle，解析 S3 XML，翻译为 GCS JSON/SDK，更新桶属性；GET 返回 S3 XML。
- CORS：PUT/GET/DELETE ?cors，解析 S3 XML，翻译为 GCS CORS，更新桶属性；GET 返回 S3 XML。
- 日志：PUT/GET/DELETE ?logging，解析 S3 XML，翻译为 GCS Logging，更新桶属性；GET 返回 S3 XML。
- 网站：PUT/GET/DELETE ?website，解析 S3 XML，翻译为 GCS Website，更新桶属性；GET 返回 S3 XML。
- 标签：PUT/GET/DELETE ?tagging，解析 S3 XML，翻译为 GCS Metadata，更新对象元数据；GET 返回 S3 XML。

**章节来源**
- [main.go:400-473](file://main.go#L400-L473)
- [main.go:500-606](file://main.go#L500-L606)
- [main.go:608-700](file://main.go#L608-L700)
- [main.go:785-872](file://main.go#L785-L872)
- [main.go:874-1016](file://main.go#L874-L1016)

### GCS API调用计时与监控
- **timeGCSCall函数**：封装所有GCS SDK调用，自动记录执行时间和错误信息。
- **指标记录**：使用 gcsAPIDuration 直方图记录各类GCS操作的执行时间。
- **结构化日志**：成功和失败的API调用都会输出结构化日志，包含操作名称、持续时间、错误信息等。
- **请求上下文**：所有GCS调用都使用请求上下文，支持客户端取消时的自动中断。

**章节来源**
- [main.go:373-386](file://main.go#L373-L386)

## 依赖关系分析
- 第三方依赖：cloud.google.com/go/storage（GCS SDK）、github.com/go-chi/chi/v5（HTTP 路由）、github.com/joho/godotenv（.env 加载）、github.com/prometheus/client_golang（Prometheus 客户端）。
- 版本约束：Go 1.25+，确保与最新云 SDK 兼容。
- 内部依赖：main.go 依赖 config 与 pkg/translate；翻译器模块相互独立，仅依赖 GCS SDK 类型。

```mermaid
graph LR
M["main.go"] --> CFG["config/settings.go"]
M --> TR["pkg/translate/*"]
M --> CHI["github.com/go-chi/chi/v5"]
M --> DOTENV["github.com/joho/godotenv"]
M --> PROM["github.com/prometheus/client_golang"]
TR --> GCS["cloud.google.com/go/storage"]
```

**图示来源**
- [go.mod:5-9](file://go.mod#L5-L9)
- [main.go:3-32](file://main.go#L3-L32)

**章节来源**
- [go.mod:1-68](file://go.mod#L1-L68)
- [main.go:3-32](file://main.go#L3-L32)

## 性能与可靠性特性
- 连接池优化：http.Transport 设置 MaxIdleConns、MaxIdleConnsPerHost、IdleConnTimeout、TLSHandshakeTimeout、ExpectContinueTimeout，禁用压缩以保留 S3 签名所需头，启用 HTTP/2 提升多路复用。
- 优雅关闭：监听 SIGTERM/SIGINT，最多等待 10 秒完成现有请求，失败则强制关闭。
- 超时控制：传输层超时参数防止挂起；GCS SDK 调用使用请求上下文，可随请求取消而终止。
- DryRun：禁用真实 GCS API 调用，便于本地测试与验证翻译逻辑。
- 版本互操作：在请求头注入版本互操作标志，在响应头映射版本号，保证 S3 客户端行为一致。
- **结构化日志**：使用 log/slog 输出结构化JSON日志，支持Info和Debug级别，便于云平台日志聚合和分析。
- **Prometheus指标**：自动收集HTTP请求、请求持续时间、GCS API调用时长等关键指标，支持外部监控系统。
- **监控端点**：/metrics 端点暴露所有注册指标，支持Prometheus抓取和可视化。

**更新** 新增了完整的可观测性体系，包括结构化日志、Prometheus指标收集和监控端点，为生产环境提供了强大的监控和诊断能力。

**章节来源**
- [main.go:101-125](file://main.go#L101-L125)
- [main.go:280-312](file://main.go#L280-L312)
- [main.go:126-229](file://main.go#L126-L229)
- [README.md:169-170](file://README.md#L169-L170)

## 故障排查指南
- 启动失败：检查 STORAGE_BASE_URL、TARGET_BUCKET、GCP 项目与凭据配置；确认 .env 是否存在且字段正确。
- 代理无响应：确认 PORT 开放、防火墙策略；查看日志中是否有反向代理错误或 GCS SDK 调用失败。
- 签名问题：若未设置 PROXY_AWS_ACCESS_KEY_ID/PROXY_AWS_SECRET_ACCESS_KEY，Director 中会跳过重签，导致 GCS 认证失败。
- CORS/网站/日志/标签更新无效：确认 DryRun=false 且 JSON_KEY 路径有效；检查 GCS 权限与桶属性更新返回状态。
- 生命周期翻译失败：检查 S3 XML 结构与过滤条件；ObjectSizeGreaterThan/LessThan、Tag 过滤不被支持。
- 标签冲突：对象标签更新使用 OCC，若出现冲突（如 412），需重试读取最新元数据后再次更新。
- **可观测性问题**：检查 DEBUG_LOGGING 环境变量设置；确认 /metrics 端点可达；验证Prometheus指标是否正常注册。
- **日志分析**：使用结构化JSON日志，通过 request_id 关联同一请求的完整处理链路；在Debug模式下查看更多细节信息。

**更新** 新增了可观测性相关的故障排查指导，包括日志分析、指标监控和监控端点检查。

**章节来源**
- [config/settings.go:29-57](file://config/settings.go#L29-L57)
- [main.go:126-229](file://main.go#L126-L229)
- [main.go:400-473](file://main.go#L400-L473)
- [main.go:874-1016](file://main.go#L874-L1016)
- [README.md:169-170](file://README.md#L169-L170)

## 结论
本项目通过清晰的分层架构与完善的翻译器模块，实现了 S3 与 GCS 在协议与语义层面的互通。反向代理负责透明转发与必要的头部重写/重签，扩展请求处理器通过翻译器将 S3 XML 映射到 GCS SDK/JSON，从而在不修改客户端代码的前提下，实现对 GCS 的完整能力覆盖。

**更新** 新增的可观测性体系进一步增强了系统的生产可用性，包括结构化日志、Prometheus指标收集和监控端点，为运维团队提供了强大的监控和诊断工具。配合连接池优化、优雅关闭与超时控制，系统具备良好的生产可用性与可观测性。

## 附录
- 集成测试：integration_tests 子模块使用独立 go.mod，通过 AWS S3 SDK 自动化验证代理行为，无需污染主模块。
- 环境变量参考：README.md 提供完整配置项说明与使用示例。
- **监控指标参考**：README.md 详细列出了所有可观测性指标的含义和用途。

**章节来源**
- [README.md:169-175](file://README.md#L169-L175)
- [integration_tests/test_utils.go:9-112](file://integration_tests/test_utils.go#L9-L112)